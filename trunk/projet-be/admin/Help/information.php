...
{	
	...
	if (! isset($_GET['titre']))
	{
		...
		// suppression
		print("\t\t\t<br><li><b>Supprimer une(des) information(s)</b><br>\n") ;
	...
	{
		dbConnect() ;
		$info = dbQuery('SELECT titre
			FROM information
			WHERE `titre` = '.$_GET['titre']) ;
			
		$infoDetails = mysql_fetch_array($info) ;
		$contenu = nl2br($infoDetails['contenu']) ;
		
		print("\t\t\t<b>{$infoDetails['titre']}</b>\n") ;
		print("\t\t\t$contenu n") ;
	}
